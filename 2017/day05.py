...
the next one. Start at the first instruction in the list. The
...
forward, but change it to a 4 for the next time it is
encountered.
...
be written all on one line, with the current instruction marked
in parentheses. The following steps would be taken before an
exit is found:
...
- (1) 3  0  1  -3  - jump with offset 0 (that is, don't jump at all).
                     ...
                     to 1.
-  2 (3) 0  1  -3  - step forward because of the instruction we
                     just modified. The first instruction is
...
def count_steps(instructions: list, update: Callable = lambda n: n + 1) -> int:
